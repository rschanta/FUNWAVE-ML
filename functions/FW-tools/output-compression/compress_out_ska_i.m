...
%% Calculate skew and asymmetry right away
    ...
    results.asy = ska.asy;
%% Save out the whole results structure
    name = fullfile(paths.outputs_proc,['out_',tri_no_str,'.mat']);
    save(name,'-struct', 'results', '-v7.3')
%% Save out just the ska structure
    name_ska = fullfile(paths.outputs_proc,['ska_',tri_no_str,'.mat']);
    save(name_ska,'-struct', 'ska', '-v7.3')
end