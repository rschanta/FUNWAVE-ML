...
function Spectra_D3_1(super_path,run_name)
    %{
    RUN SCRIPT
    ...
    %% Load in Data for Trial 5
        ...
        tri = 3; all_inputs = struct();

    %{
        First Five Trials: Pure eta signal at left boundary, set velocities to 0.
    ...
    for DX = linspace(0.4,0.5,2)
        ...
        %%% GET SPECTRAL DATA FOR TRIAL, set n_bins and f_max
            spectra = get_D3_spectra(Trial,35,0.95);
        ...
        %%% MAKE FWS STRUCTURE AND SET INPUTS
            input = FW_in_LEFT_BC_IRR();
                ...
                input.TOTAL_TIME = 600;
                input.PLOT_INTV = 0.01;
                input.DX = DX;
                input.DEPTH_FILE = ptr.b_file;
                input.WaveCompFile = ptr.s_file;
        ...
        %%% PRINT INPUT AND STORE TO STRUCTURE
            ...
            % Print spectra
                ...
                print_1D_spectra(spectra,ptr.s_file);
                disp(['spectra_',ptr.num_str,'.txt successfully saved to',ptr.s_file]); 
